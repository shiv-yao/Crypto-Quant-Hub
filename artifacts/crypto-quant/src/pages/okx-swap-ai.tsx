...
type DecisionResult = "opened" | "blocked" | "hold";
...
function decisionBadge(result: DecisionResult) {
  if (result === "opened") return "bg-emerald-500/10 border-emerald-500/40 text-emerald-400";
  if (result === "blocked") return "bg-amber-500/10 border-amber-500/40 text-amber-400";
  return "bg-slate-500/10 border-slate-500/40 text-slate-400";
}

function Plan({ plan }: { plan: AdaptivePlan }) {
  return (
    <div className="mt-2 grid grid-cols-2 gap-1 text-xs font-mono text-muted-foreground sm:grid-cols-4">
      <span>倉位 {plan.notionalUsdt} USDT</span>
      <span>槓桿 {plan.leverage}x</span>
      <span>SL {plan.stopLossPct}%</span>
      <span>TP {plan.takeProfitPct}%</span>
      <span>追蹤 {plan.trailingStopPct}%</span>
      <span>持倉 {plan.maxHoldMinutes} 分</span>
      <span>反向出場 {plan.exitOnOppositeConfidence}%</span>
      <span>波動 {plan.volatilityRegime}</span>
    </div>
  );
}
...
export default function OkxSwapAi() {
  ...
  async function load() {
    ...
    try {
      ...
      setState(data);
      setMessage(data.enabled ? "OKX 合約 AI Demo 勝率優先引擎執行中" : "OKX 合約 AI Demo 引擎目前停止");
    } catch (error) {
  ...
  const blockedReasons = useMemo(
    () => Object.entries(state?.blockedReasonCounts ?? {}).sort((left, right) => right[1] - left[1]),
    [state?.blockedReasonCounts],
  );
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-foreground">OKX 合約 AI 勝率優先版</h1>
          <p className="mt-1 text-xs text-muted-foreground">Top 成交量合約 · 5m + 15m 同向確認 · 動態止盈止損 · 連虧冷卻 · Demo Trading</p>
        </div>
      ...
      <div className="rounded-lg border border-primary/30 bg-primary/5 p-4 text-xs text-muted-foreground leading-relaxed">
        <div className="flex items-start gap-2">
          ...
          <span>AI 有方向訊號後，仍必須通過 5m 與 15m 同向、趨勢強度、動能、量能、ATR、RSI、每日停損與連虧冷卻檢查才會建立 Demo 倉位。交易數可能下降，但低品質交易會被擋下。</span>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Stat label="全部 USDT 合約" value={state?.universeCount ?? 0} sub="OKX 動態探索" />
        <Stat label="可交易合約池" value={state?.eligibleUniverseCount ?? 0} sub={`成交量 Top ${state?.topVolumeLimit ?? 0}`} />
        <Stat label="原始 AI 訊號" value={state?.signalCount ?? 0} sub="尚未通過品質過濾" />
        <Stat label="合格進場訊號" value={state?.qualifiedSignalCount ?? 0} sub="已通過多時間框架與風控" />
        <Stat label="目前持倉" value={state?.positions.length ?? 0} sub={`筆數上限 ${state?.positionLimit ?? "不限"}`} />
        <Stat label="今日已實現 PnL" value={`${(state?.dailyRealizedPnlPct ?? 0).toFixed(2)}%`} sub={`每日停損 -${state?.dailyStopPct ?? 0}%`} />
        <Stat label="連續虧損" value={state?.consecutiveLosses ?? 0} sub={cooldownActive ? "目前冷卻中" : "未觸發冷卻"} />
        <Stat label="累積已實現 PnL" value={`${(state?.realizedPnlPct ?? 0).toFixed(2)}%`} />
        <Stat label="最大總曝險" value={`${state?.maxExposureUsdt ?? 0} USDT`} />
        <Stat label="累積掃描" value={state?.scannedCount ?? 0} sub={`每 ${Math.round((state?.intervalMs ?? 30000) / 1000)} 秒`} />
      ...
      <section className="rounded-lg border border-border bg-card p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-4">
          ...
          <button disabled={loading} onClick={() => void action("run-once", "已完成一輪 AI 掃描")} className="flex items-center justify-center gap-2 rounded-md border border-primary/40 bg-primary/10 px-3 py-2 text-xs font-semibold text-primary disabled:opacity-50"><Zap size={13} /> 立即掃描</button>
        ...
        {state?.cooldownUntil && cooldownActive && <div className="rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-300">連虧冷卻中，暫停建立新倉至 {new Date(state.cooldownUntil).toLocaleString("zh-TW")}</div>}
      ...
      <section className="rounded-lg border border-border bg-card p-4 space-y-3">
        <div className="flex items-center gap-2"><Bug size={14} className="text-primary" /><h2 className="text-sm font-semibold text-foreground">為什麼沒下單 Debug</h2></div>
        {!blockedReasons.length ? <p className="text-xs text-muted-foreground">目前尚無被擋下的訊號。啟動引擎後會自動統計原因。</p> : (
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {blockedReasons.map(([reason, count]) => <div key={reason} className="rounded-md border border-border/60 bg-background p-3 text-xs"><p className="font-semibold text-foreground">{reason}</p><p className="mt-1 font-mono text-muted-foreground">累積 {count} 次</p></div>)}
          </div>
        )}
        <div className="space-y-2">
          {(state?.recentDecisions ?? []).slice(0, 20).map((decision, index) => (
            <div key={`${decision.time}-${decision.instId}-${index}`} className="rounded-md border border-border/60 bg-background p-3 text-xs">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="font-semibold text-foreground">{decision.instId} · {decision.action} · 分數 {decision.score.toFixed(1)}</span>
                <span className={`rounded-full border px-2 py-0.5 font-semibold ${decisionBadge(decision.result)}`}>{decision.result === "opened" ? "已建立倉位" : decision.result === "blocked" ? "已擋下" : "等待"}</span>
              </div>
      ...
      <section className="rounded-lg border border-border bg-card p-4 space-y-3">
        <div className="flex items-center gap-2"><Gauge size={14} className="text-primary" /><h2 className="text-sm font-semibold text-foreground">AI 訊號與動態計畫</h2></div>
        {!state?.latestSignals.length ? <p className="text-xs text-muted-foreground">尚無訊號，請啟動引擎或立即掃描。</p> : state.latestSignals.map((signal) => (
          <div key={`${signal.instId}-${signal.generatedAt}`} className="rounded-md border border-border/60 bg-background p-3">
            <div className="flex items-center justify-between gap-2"><p className="text-xs font-bold text-foreground">{signal.instId}</p><span className={`rounded-full border px-2 py-0.5 text-xs font-semibold ${badge(signal.action)}`}>{signal.action} · {signal.score.toFixed(1)}</span></div>
            <p className="mt-2 text-xs font-mono text-muted-foreground">原始 {signal.rawAction} · 信心 {signal.confidence}% · 5m {signal.trendPct}% · 15m {signal.trend15mPct}% · RSI {signal.rsi} · ATR {signal.atrPct}% · 動能 {signal.momentumPct}% · 量能 {signal.volumeRatio}x</p>
            <Plan plan={signal.plan} />
            {!!signal.blockedReasons.length && <p className="mt-2 text-xs text-amber-300">擋下原因：{signal.blockedReasons.join(" · ")}</p>}
            <p className="mt-2 text-xs text-muted-foreground">{[...signal.reasons, ...signal.plan.reasons].join(" · ")}</p>
          </div>
        ))}
      </section>

      <section className="rounded-lg border border-border bg-card p-4 space-y-3">
        <div className="flex items-center gap-2"><Activity size={14} className="text-primary" /><h2 className="text-sm font-semibold text-foreground">AI 管理中的合約持倉</h2></div>
        {!state?.positions.length ? <p className="text-xs text-muted-foreground">目前沒有持倉。</p> : state.positions.map((position) => (
          <div key={position.id} className="rounded-md border border-border/60 bg-background p-3 text-xs">
            ...
            <p className={`mt-1 font-mono ${position.unrealizedPnlPct >= 0 ? "text-emerald-400" : "text-red-400"}`}>PnL {position.unrealizedPnlPct.toFixed(2)}% · Peak {position.peakFavorablePnlPct.toFixed(2)}%</p>
            <p className="mt-1 font-mono text-muted-foreground">SL {position.stopLossPct}% · TP {position.takeProfitPct}% · 追蹤 {position.trailingStopPct}% · 最大 {position.maxHoldMinutes} 分 · 反向出場 {position.exitOnOppositeConfidence}% · AI {position.lastAiScore}</p>
          </div>
        ))}
      </section>
        ...
        {(state?.skills ?? []).map((skill) => (
          <div key={skill.id} className="rounded-md border border-border/60 bg-background p-3">
            <div className="flex items-start justify-between gap-3">
              <div><p className="text-xs font-semibold text-foreground">{skill.name}</p><p className="mt-1 text-xs text-muted-foreground">{skill.description}</p><p className="mt-1 text-xs font-mono text-muted-foreground">{skill.id}</p></div>
